      ...
      if (res.data.code === 200) {
        const results = res.data.results
          .map(item => {
            try {
              // 打印 detail 的原始值
              // console.log("Raw detail data:", item.detail);
              ...
              const rawDetail = JSON.parse(fixedDetail); // 尝试解析修复后的 JSON 字符串
              ...
              // 如果解析失败，就忽略该条数据
              console.error("Failed to parse item detail as JSON:", item.detail, error);
              return null;
            }
          })
          .filter(item => item !== null); // 过滤掉返回 null 的条目
